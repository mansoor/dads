...
_backup_postgres() {
  local container="$1" db_user="$2" db_name="$3" label="${4:-postgres}"
...
_backup_mysql() {
  local container="$1" root_pass="$2" db_name="$3" label="${4:-mysql}"
  local file="$BACKUP_DIR/${PROJECT}_${ENV}_${label}_${DATE_DIR}.sql.gz"
  docker exec "$container" mysqldump -u root -p"$root_pass" "$db_name" | gzip > "$file"
  ...
  if [[ "$PROJECT_TYPE" == "image" ]]; then
    ...
    while [[ $idx -lt $img_count ]]; do
      ...
      if [[ "$img_name" == *"postgres"* ]]; then
        found_db=true
        log_info "Found PostgreSQL container: $container"
        local pg_user="${POSTGRES_USER:-postgres}"
        local pg_db="${POSTGRES_DB:-${PROJECT}}"
        _backup_postgres "$container" "$pg_user" "$pg_db" "${svc_name}"

      elif [[ "$img_name" == *"mysql"* || "$img_name" == *"mariadb"* ]]; then
        found_db=true
        log_info "Found MySQL/MariaDB container: $container"
        local my_pass="${MYSQL_ROOT_PASSWORD:-}"
        local my_db="${MYSQL_DATABASE:-${PROJECT}}"
        if [[ -z "$my_pass" ]]; then
          log_warn "MYSQL_ROOT_PASSWORD not set in .env — skipping $container"
          continue
        fi
        _backup_mysql "$container" "$my_pass" "$my_db" "${svc_name}"
      fi
    done
    ...
    if [[ "$DATABASE" == "postgres" ]]; then
      _backup_postgres "${PREFIX}_postgres" "$POSTGRES_USER" "$POSTGRES_DB"

    elif [[ "$DATABASE" == "mysql" ]]; then
      _backup_mysql "${PREFIX}_mysql" "$MYSQL_ROOT_PASSWORD" "$MYSQL_DATABASE"
